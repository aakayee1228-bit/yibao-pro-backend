...
const QuoteDetailPage: FC = () => {
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      ...
      // 标题
      ctx.fillStyle = '#ffffff'
      ctx.font = 'normal bold 48px system-ui'
      ctx.textAlign = 'center'
      ctx.fillText('产品报价单', 375, 78)

      // ========== 两列信息布局 ==========
      let yPos = 140
      ctx.font = 'normal normal 24px system-ui'

      // 左列 - 客户信息
      ctx.fillStyle = '#1f2937'
      ctx.font = 'normal bold 28px system-ui'
      ctx.textAlign = 'left'
      const customerName = quote.customers?.name || '客户'
      ...
      // 表头文字
      ctx.fillStyle = '#ffffff'
      ctx.font = 'normal bold 24px system-ui'
      ctx.textAlign = 'center'
      ...
      if (quote.items && quote.items.length > 0) {
        quote.items.forEach((item, index) => {
          ...
          ctx.font = 'normal normal 22px system-ui'
          ctx.textAlign = 'center'
          ...
          // 合计
          ctx.fillStyle = '#1e40af'
          ctx.font = 'normal bold 22px system-ui'
      ...
      ctx.fillStyle = '#1e40af'
      ctx.font = 'normal bold 32px system-ui'
      ...
      yPos = 1350
      ctx.fillStyle = '#9ca3af'
      ctx.font = 'normal normal 20px system-ui'
      ctx.textAlign = 'center'
      ...
      ctx.fillStyle = '#1e40af'
      ctx.font = 'normal bold 36px system-ui'
      ctx.textAlign = 'center'
