...
const EditQuotePage: FC = () => {
  ...
  const fetchQuoteDetail = async (id: string) => {
    ...
    try {
      ...
      console.log('获取表单详情响应:', res.data)

      if (res.data?.data) {
        const data = res.data.data as QuoteDetail
            ...
            {items.length === 0 ? (
              ...
              <View className="flex flex-col gap-3">
                {items.map((item, index) => (
                  <View key={item.id || index} className="p-3 bg-gray-50 rounded-lg">
